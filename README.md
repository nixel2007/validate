...
- `&Размер(Минимум = <число>, Максимум = <число>)` — размер значения в заданных границах. Код нарушения: `size`.
- `&Кратно(<число>)` — значение кратно указанному числу. Код нарушения: `multipleOf`.
- `&Шаблон(<строка>)` — значение соответствует регулярному выражению. Код нарушения: `pattern`.
- `&Формат(<строка>)` — значение соответствует известному формату. Код нарушения: `format`.
- `&ОдинИз(<строка>)` — значение входит в список допустимых. Код нарушения: `oneOf`.
...
`&Минимум` и `&Максимум` сравнивают само значение, `&Размер` — его размер: длину строки либо количество элементов коллекции. Для строки `&Максимум(10)` означает лексикографическое сравнение, а `&Размер(Максимум = 10)` — ограничение длины. Ноль в `Максимум` означает отсутствие верхней границы.

`&Шаблон` проверяет значение целиком: выражение неявно привязано к началу и концу строки, поэтому `\d+` не пропустит `abc123`.

У `&ОдинИз` параметр `Значение` повторяемый: допустимые значения перечисляются повторением параметра. Сравнение строковое, поэтому ограничение работает и для чисел.

`&Кратно` допускает дробную кратность: `&Кратно(0.5)` пропустит `1.5` и не пропустит `1.2`.

`&Формат` знает `email`, `uuid`, `uri`, `date`, `date-time` и `ipv4`. Список намеренно открытый: неизвестное имя формата значение не отбраковывает — формат это ещё и способ описать поле для потребителя, и ограничивать разметку тем, что библиотека умеет проверять, незачем. Выражения нестрогие: их задача — отсечь явно непохожее, а не заменить собой разбор адреса или даты.

```bsl
&Размер(Минимум = 2, Максимум = 20)
Перем Имя Экспорт;
