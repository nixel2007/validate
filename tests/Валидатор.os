...
&Тест
Процедура ВалидацияОграниченияШаблонРаботает() Экспорт

	// Дано
	Валидатор = Новый Валидатор;
	ТестовыйОбъект = ЗаполненныйТестовыйОбъект();
	ТестовыйОбъект.ПолеСШаблоном = "12-345";
	...
	Нарушение = РезультатВалидации[0];
	Ожидаем.Что(Нарушение.ИмяСвойства()).Равно("ПолеСШаблоном");
	Ожидаем.Что(Нарушение.Код()).Равно("pattern");

КонецПроцедуры

&Тест
Процедура ВалидацияОграниченияШаблонПропускаетПодходящее() Экспорт

	// Дано
	Валидатор = Новый Валидатор;
	ТестовыйОбъект = ЗаполненныйТестовыйОбъект();
	ТестовыйОбъект.ПолеСШаблоном = "123-45";
...
// Выражение проверяет значение целиком, а не ищет совпадение внутри: иначе
// шаблон пропустил бы значение с посторонними символами вокруг.
&Тест
Процедура ВалидацияОграниченияШаблонПроверяетЗначениеЦеликом() Экспорт

	// Дано
	Валидатор = Новый Валидатор;
	ТестовыйОбъект = ЗаполненныйТестовыйОбъект();
	ТестовыйОбъект.ПолеСШаблоном = "абв123-45где";
...
&Тест
Процедура ВалидацияОграниченияРазмерРаботаетДляСтроки() Экспорт

	// Дано
	Валидатор = Новый Валидатор;
	ТестовыйОбъект = ЗаполненныйТестовыйОбъект();
	ТестовыйОбъект.ПолеСРазмером = "слишком длинное";
	...
	Нарушение = РезультатВалидации[0];
	Ожидаем.Что(Нарушение.ИмяСвойства()).Равно("ПолеСРазмером");
	Ожидаем.Что(Нарушение.Сообщение()).Равно("Размер значения вне диапазона <2..5>");
	Ожидаем.Что(Нарушение.Код()).Равно("size");

КонецПроцедуры

// Размером коллекции считается количество элементов, а не длина её
// строкового представления.
&Тест
Процедура ВалидацияОграниченияРазмерРаботаетДляКоллекции() Экспорт

	// Дано
	Валидатор = Новый Валидатор;
	ТестовыйОбъект = ЗаполненныйТестовыйОбъект();
	ТестовыйОбъект.КоллекцияСРазмером = Новый Массив();
	ТестовыйОбъект.КоллекцияСРазмером.Добавить("раз");
	ТестовыйОбъект.КоллекцияСРазмером.Добавить("два");
	ТестовыйОбъект.КоллекцияСРазмером.Добавить("три");
	...
	// Тогда
	Ожидаем.Что(РезультатВалидации).ИмеетДлину(1);
	Ожидаем.Что(РезультатВалидации[0].ИмяСвойства()).Равно("КоллекцияСРазмером");
...
&Тест
Процедура ВалидацияОграниченияРазмерБезВерхнейГраницы() Экспорт

	// Дано
	Валидатор = Новый Валидатор;
	ТестовыйОбъект = ЗаполненныйТестовыйОбъект();
	ТестовыйОбъект.ПолеСРазмеромБезВерхнейГраницы = "очень длинное значение без ограничения";
...
&Тест
Процедура ВалидацияОграниченияОдинИзРаботает() Экспорт

	// Дано
	Валидатор = Новый Валидатор;
	ТестовыйОбъект = ЗаполненныйТестовыйОбъект();
	ТестовыйОбъект.ПолеИзСписка = "удалён";
	...
	Нарушение = РезультатВалидации[0];
	Ожидаем.Что(Нарушение.ИмяСвойства()).Равно("ПолеИзСписка");
	Ожидаем.Что(Нарушение.Сообщение())
		.Равно("Значение не входит в список допустимых <создан, оплачен, отгружен>");
	Ожидаем.Что(Нарушение.Код()).Равно("oneOf");

КонецПроцедуры

&Тест
Процедура ВалидацияОграниченияОдинИзПропускаетДопустимое() Экспорт

	// Дано
	Валидатор = Новый Валидатор;
	ТестовыйОбъект = ЗаполненныйТестовыйОбъект();
	ТестовыйОбъект.ПолеИзСписка = "отгружен";
...
&Тест
Процедура ВалидацияОграниченияКратноРаботает() Экспорт

	// Дано
	Валидатор = Новый Валидатор;
	ТестовыйОбъект = ЗаполненныйТестовыйОбъект();
	ТестовыйОбъект.ПолеКратное = 1.2;
	...
	Нарушение = РезультатВалидации[0];
	Ожидаем.Что(Нарушение.ИмяСвойства()).Равно("ПолеКратное");
	Ожидаем.Что(Нарушение.Код()).Равно("multipleOf");

КонецПроцедуры

// Кратность бывает дробной, поэтому проверка идёт делением, а не остатком:
// остаток от деления дробных чисел ведёт себя неочевидно.
&Тест
Процедура ВалидацияОграниченияКратноПропускаетДробноеКратное() Экспорт

	// Дано
	Валидатор = Новый Валидатор;
	ТестовыйОбъект = ЗаполненныйТестовыйОбъект();
	ТестовыйОбъект.ПолеКратное = 1.5;
...
&Тест
Процедура ВалидацияОграниченияФорматРаботает() Экспорт

	// Дано
	Валидатор = Новый Валидатор;
	ТестовыйОбъект = ЗаполненныйТестовыйОбъект();
	ТестовыйОбъект.ПолеПочты = "не почта";
	...
	Нарушение = РезультатВалидации[0];
	Ожидаем.Что(Нарушение.ИмяСвойства()).Равно("ПолеПочты");
	Ожидаем.Что(Нарушение.Сообщение()).Равно("Значение не соответствует формату <email>");
	Ожидаем.Что(Нарушение.Код()).Равно("format");

КонецПроцедуры

&Тест
Процедура ВалидацияОграниченияФорматПропускаетПодходящее() Экспорт

	// Дано
	Валидатор = Новый Валидатор;
	ТестовыйОбъект = ЗаполненныйТестовыйОбъект();
	ТестовыйОбъект.ПолеПочты = "почта@example.com";
	ТестовыйОбъект.ПолеИдентификатора = "3f2504e0-4f89-41d3-9a0c-0305e82c3301";
...
// Список форматов открытый: неизвестное имя значение не отбраковывает, потому
// что разметка полезна как описание поля даже там, где проверить его нечем.
&Тест
Процедура ВалидацияОграниченияФорматПропускаетНеизвестныйФормат() Экспорт

	// Дано
	Валидатор = Новый Валидатор;
	ТестовыйОбъект = ЗаполненныйТестовыйОбъект();
	ТестовыйОбъект.ПолеНеизвестногоФормата = "что угодно";
...
// разные значения.
&Тест
Процедура ВалидацияОграниченияОдинИзСравниваетЗначенияАНеПредставления() Экспорт

	// Дано
	Валидатор = Новый Валидатор;
	ТестовыйОбъект = ЗаполненныйТестовыйОбъект();
	ТестовыйОбъект.ПолеИзЧисел = "1";
	...
	// Тогда
	Ожидаем.Что(РезультатВалидации).ИмеетДлину(1);
	Ожидаем.Что(РезультатВалидации[0].ИмяСвойства()).Равно("ПолеИзЧисел");

КонецПроцедуры

&Тест
Процедура ВалидацияОграниченияОдинИзРаботаетДляЧисел() Экспорт

	// Дано
	Валидатор = Новый Валидатор;
	ТестовыйОбъект = ЗаполненныйТестовыйОбъект();
	ТестовыйОбъект.ПолеИзЧисел = 2;
...
// Шаблон описывает вид строки, поэтому нестроковое значение проверку
// не проходит: приводить его к строке значило бы проверять представление,
// а оно зависит от настроек.
&Тест
Процедура ВалидацияОграниченияШаблонОтклоняетНестроку() Экспорт

	// Дано
	Валидатор = Новый Валидатор;
	ТестовыйОбъект = ЗаполненныйТестовыйОбъект();
	ТестовыйОбъект.ПолеСШаблоном = 12345;
...
// Неверная разметка - ошибка разработчика, и обнаружиться она должна при
// разборе аннотации, а не молча превратиться в нарушение у каждого значения.
&Тест
Процедура АннотацияКратноОтвергаетНоль() Экспорт

	ОшибкаВозникла = Ложь;

	Попытка
		АннотацияКратно = Новый АннотацияКратно(0);
	Исключение
...
&Тест
Процедура АннотацияКратноОтвергаетНеЧисло() Экспорт

	ОшибкаВозникла = Ложь;

	Попытка
		АннотацияКратно = Новый АннотацияКратно("половина");
	Исключение
		ОшибкаВозникла = Истина;
	КонецПопытки;
...
// Обязательные поля заполнены, чтобы в результате оставались только нарушения
// проверяемого ограничения.
Функция ЗаполненныйТестовыйОбъект()
